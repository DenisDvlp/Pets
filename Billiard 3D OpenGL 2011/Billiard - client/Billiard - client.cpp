...
bool rkm,LKM,lkm,blw,bls,bla,bld,menu1,menu2,menu3,new1,cont,exit1,yes,no,menu11,menu22,menu33;
GLuint	texture[6];
bool ifcross[120],showball[16],overforce,ifblack[6],isActive; 
...
void SEND(bool first)
{
	...
	if(first)
	{
		for(i=0;i<300;i++)
		buf[i]=NULL;
		itoa(ball[selected].angle,buf,10);
		buf[strlen(buf)]=' ';
		itoa(selected,bf,10);
		strcat(buf,bf);
		for(j=0;j<32;j++)bf[j]=NULL;
		prom=ball[selected].acceleration*10000;
		itoa(prom,bf,10);
		buf[strlen(buf)]=' ';
		strcat(buf,bf);
		send(s2,buf,300,0);
...
void RECV()
{
	...
	recv(s2,buf,300,0);
		if(buf[0]!=NULL)
		{
			...
			if(!LKM)
			{	
				cout<<"First player says:"<<endl;
				for(i=0;i<strlen(buf);i++)
				{
					if(buf[i]==' ')break;
				}
				selected=atoi(buf+i);
				ball[selected].angle=atoi(buf);
				k=0;j=0;
				while(k<2)
				{
					if(buf[j]==' ')k++;j++;
				}
				ball[selected].acceleration=(float)atoi(buf+j);
				ball[selected].acceleration/=10000;
				LKM=true;
				cout<<"selected="<<selected<<";Angle="<<ball[selected].angle<<";Acceleration="<<ball[selected].acceleration<<endl;
			}
			else
			{
				for(i=0;i<16;i++)
				{
					...
					ball[i].z=(float)atoi((buf+j));
					ball[i].z/=1000000;
					cout<<i<<")"<<ball[i].x<<" "<<ball[i].z<<endl;
				}
				isActive=true;
				LKM=false;
			}
...
int main( HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmd, int nShowCmd)
{
	...
	double length=0.08f, movement=0.03f, deaccel=0.001f, grad=3.14159265/180,agrad=180/3.14159265;
	int gamai,gamaj,beta;
	FILE * fi;
	...
	wc.lpszClassName	= "Billiard_Petrov";

	RegisterClass(&wc); 
	hWnd = CreateWindow("Billiard_Petrov","Billiard by D. Petrov",WS_POPUP |WS_OVERLAPPEDWINDOW |WS_CLIPSIBLINGS, -8, -30, 
	...
	char string[16];
    while( !feof(fi) ) fscanf( fi, "%s", string ); 
    fclose(fi);
    ...
    adress.sin_port=htons(1280);
    adress.sin_addr.s_addr=inet_addr(string);

	ioctlsocket(s2,FIONBIO,(unsigned long* ) &l);
	Draw();
	while(1)
	{
	...
	{connect(s2,(sockaddr *)&adress,sizeof(adress));
///вычисления///
			if(blw)	{	
