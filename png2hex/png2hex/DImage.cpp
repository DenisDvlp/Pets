...
bool DImage::load(std::string filePath)
{
  ...
  // Read the signature.
  const size_t byteRead = fread_s(header, signatureSize, 1, signatureSize, file);
...
DColor* const DImage::operator[](size_t index)
{
    return m_pixels[index];
}

const DColor* const DImage::operator[](size_t index) const
{
  return m_pixels[index];
}
