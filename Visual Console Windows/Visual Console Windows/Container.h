...
{
  remove_helper(m_items, std::forward<T>(items));
}
...
template<typename STDContainer, typename U = if_container_dynamic_linear_bidirectional_pure<STDContainer>>
void Container<STDContainer, U>::clear()
{
  m_items.clear();
}
...
{
  remove_helper2(container, std::forward<T2>(items), is_value_type<T2>());
}
...
{
  container.erase(std::remove(container.begin(), container.end(), std::forward<T2>(items)), container.end());
}
...
{
  container.remove(std::forward<T>(items));
}
...
{
  remove_helper3(container, std::forward<T2>(items), is_predicate1<T2>());
}
...
{
  container.erase(std::remove_if(container.begin(), container.end(), std::forward<T2>(predicate)), container.end());
}
...
{
  container.remove_if(std::forward<T>(predicate));
}
...
void Container<STDContainer, U>::remove_helper3(T1& container, T2&& items, boolean_false)
{
  ...
  auto it = find_helper2(container.begin(), container.end(), std::forward<T2>(items));
...
{
  container.sort(std::forward<Predicate>(predicate));
}
...
{
  return std::is_sorted(container.begin(), container.end(), std::forward<Predicate>(predicate));
}
...
{
  merge_helper3(container, items.begin(), items.end(), std::forward<Predicate>(predicate));
}
