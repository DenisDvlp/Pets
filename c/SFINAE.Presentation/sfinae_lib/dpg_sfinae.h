...
template<typename T1, typename T2> constexpr bool is_convertable = is_convertable_impl<T1, T2>::value;
...
using has_signature = IF<is_same<decltype(val<T>()(val<Args>()...)), R>>;
template<typename T, typename R, typename... Args>
struct is_callable_match_impl : False {};
template<typename T, typename R, typename... Args>
struct is_callable_match_impl<T, R(Args...)> : feature_detector<has_signature, T, R, Args...> {};
template <typename T, typename F>
constexpr bool is_callable_match = is_callable_match_impl<T, F>::value;
...
constexpr bool check_iterator = is_convertable<TAG, T>;
...
template<typename T> using has_method_sort_impl = decltype(&T::sort);
template<typename T> constexpr bool has_method_sort = feature_detected<has_method_sort_impl, T>;
