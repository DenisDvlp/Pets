...
static constexpr bool is_type_compatible_v = std::is_same_v<std::remove_cv_t<Type>, std::remove_cv_t<Type2>> && (std::is_const_v<Type> || !std::is_const_v<Type2>);

template <typename Type, typename P>
static constexpr bool is_pointer_compatible_v = std::is_pointer_v<P>&& is_type_compatible_v<Type, std::remove_pointer_t<P>>;
...
static constexpr bool is_iterator_compatible_v<Type, I, std::enable_if_t<std::is_same_v<std::random_access_iterator_tag, typename I::iterator_category> && is_pointer_compatible_v<Type, typename I::pointer>>> = true;

template <typename Type, typename I, typename = void>
static constexpr bool is_pointer_or_iterator_compatible_v = false;
template <typename Type, typename I>
static constexpr bool is_pointer_or_iterator_compatible_v<Type, I, std::enable_if_t<is_pointer_compatible_v<Type, I> || is_iterator_compatible_v<Type, I>>> = true;
...
static constexpr bool is_container_compatible_v<Type, C, std::enable_if_t<is_pointer_compatible_v<Type, decltype(std::declval<C>().data())>>> = true;
