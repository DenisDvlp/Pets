...
Window2::Window2(const wchar_t* name, HINSTANCE hInstance) : name{name}, hInstance{hInstance} {
    WNDCLASS wc{};
...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    if (message == WM_NCCREATE) {
        ...
        // auto self =
        //     reinterpret_cast<Window2 *>(GetWindowLongPtr(hwnd,
        //     GWLP_USERDATA));
        // if(!self)
        //     assert("fuck");
    ...
    // if (message == WM_NCCREATE) {
    //     auto createStruct = reinterpret_cast<CREATESTRUCT *>(lParam);
    //     auto self = reinterpret_cast<Window2
    //     *>(createStruct->lpCreateParams); SetWindowLongPtr(hwnd,
    //     GWLP_USERDATA, (LONG_PTR)self); return DefWindowProc(hwnd, message,
    //     wParam, lParam);
    // } else {
    //     auto self =
    //         reinterpret_cast<Window2 *>(GetWindowLongPtr(hwnd,
...
Application2::Application2(HINSTANCE hInstance) : window{L"Billiards", hInstance} {}
