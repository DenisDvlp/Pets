...
template <typename T>
using is_simple_pure = boolean<is_simple_v<T> && is_exact_v<T, pure<T>>>;

template <typename T>
constexpr bool is_simple_pure_v = is_simple_pure<T>::value;

template <typename T>
using if_simple_pure = if_t<is_simple_pure_v<T>>;


template<class _Fty> class std::function;
